...
private class LoginViewController: UIViewController {
    ...
    private var currentLoginViewController: UIViewController {
        switch Segments(rawValue: loginTypeSelectView.selectedSegmentIndex)! {
        case .phone:
...
private class LoginPhoneViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    private lazy var countryCodeLabel: UILabel = {
        ...
        let tgr = UITapGestureRecognizer(target: self, action: #selector(countryCodeLabelPressed))
    ...
    @objc private func submitButtonPressed() {
        guard let countryCode = countryCodeLabel.text, countryCode.count > 0 else { showError("Must enter country code"); return }
        guard let phoneNumber = phoneNumberTextField.text, phoneNumber.count > 0 else { showError("Must enter phone number"); return }
