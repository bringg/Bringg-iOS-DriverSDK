...
//
//  Created by Ido Mizrachi on 15/08/2019.
//  Copyright © 2019 Bringg. All rights reserved.
//
...
class InventoryViewController: UIViewController {
    ...
    private func dataChanged() {
        ...
        Bringg.shared.tasksManager.getTask(withTaskId: taskId) { task, error in
            if let error = error {
                logError("Failed to fetch task \(taskId), error: \(error)")
                return
            }
