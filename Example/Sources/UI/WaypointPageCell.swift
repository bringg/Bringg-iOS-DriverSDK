...
protocol WaypointPageCellDelegate: AnyObject {
...
class WaypointPageCell: FSPagerViewCell {
...
extension WaypointPageCell: UITableViewDelegate, UITableViewDataSource {
    ...
    @nonobjc func tableView(_ tableView: UITableView, didSelectPhoneNumberCellAtIndexPath indexPath: IndexPath) {
        ...
        case .phoneData:
            ...
            guard waypoint?.phoneAvailable == true else {
                print("Phone is not available, no need to fetch from the server")
                return
            }
            ...
            Bringg.shared.tasksManager.getMaskedPhoneNumber(taskId: taskId, waypointId: waypointId) { result in
                ...
                case .failure(let error):
                    print("Failed to get masked phone number for task:\(taskId), waypoint: \(waypointId), error: \(error.localizedDescription)")
